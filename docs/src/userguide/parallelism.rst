...
For the Microsoft Visual C++ compiler, the correct flag to use OpenMP is ``'/openmp'`` instead of ``'-fopenmp'`` for the ``'extra_compile_args'`, and to prevent uncesseary warnings don't pass any arguments to ``'extra_link_args'``.
